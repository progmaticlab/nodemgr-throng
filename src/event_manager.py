...
from sandesh_common.vns.constants import (
    INSTANCE_ID_DEFAULT, Module2NodeType,
...
class EventManager(object):
    ...
    def __init__(self, config, type_info, unit_names, update_process_list=False):
        ...
        p = int(config.http_server_port) if hasattr(config, 'http_server_port') else \
    ...
    def runforever(self):
        while True:
            try:
                self.msg_log('PROCESS_INFO_MANAGER.RUN_JOB', SandeshLevel.SYS_WARN)
